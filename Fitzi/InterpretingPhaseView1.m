...
#import "InterpretingPhaseView.h"

@implementation InterpretingPhaseView1
...
- (IBAction) swipeUpDone:(id) sender
{
    ...
    for (id object in bundle) {
        if ([object isKindOfClass:[InterpretingPhaseView_Athelete class]])
            athelete = (InterpretingPhaseView_Athelete *)object;
    }
    [self addSubview:athelete];
    athelete.frame = CGRectMake(0.0, 0.0, athelete.bounds.size.width, athelete.bounds.size.height);
    [UIView animateWithDuration:1.0
                     animations:^{
                         ...
                         athelete.frame =  CGRectMake(0.0,0.0, athelete.bounds.size.width, (athelete.bounds.size.height) );//self.frame; // its final location
                     }];
}
...
- (IBAction) swipeDownDone:(id) sender
{
    ...
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"InterpretingPhaseView"
                                                    owner:self options:nil];
    InterpretingPhaseView *indexView;
    for (id object in bundle) {
        if ([object isKindOfClass:[InterpretingPhaseView class]])
            indexView = (InterpretingPhaseView *)object;
    }
