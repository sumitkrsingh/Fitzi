//
//  Interpreting_QmoPhaseRevealed1.m
//  Fitzi
//
...
@implementation Interpreting_QmoPhaseRevealed1
...
- (IBAction) swipeUpDone:(id) sender
{
    ...
    for (id object in bundle) {
        if ([object isKindOfClass:[Interpreting_QmoPhaseRevealed1_2 class]])
           qmo1_2 = (Interpreting_QmoPhaseRevealed1_2 *)object;
    }
    [self addSubview:qmo1_2];
    qmo1_2.frame = CGRectMake(0.0, self.bounds.size.height, qmo1_2.bounds.size.width, qmo1_2.bounds.size.height);
    [UIView animateWithDuration:1.0
                     animations:^{
                         ...
                         qmo1_2.frame = self.frame; // its final location
                     }];
}
...
- (IBAction) swipeDownDone:(id) sender
{
    ...
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"Interpreting_QmoPhaseRevealed"
                                                    owner:self options:nil];
    Interpreting_QmoPhaseRevealed *indexView;
    for (id object in bundle) {
        if ([object isKindOfClass:[Interpreting_QmoPhaseRevealed class]])
            indexView = (Interpreting_QmoPhaseRevealed *)object;
    }
