...
@implementation FitziProcessEvaluation14
...
- (IBAction) swipeUpDone:(id) sender
{
    ...
    for (id object in bundle) {
        if ([object isKindOfClass:[FitziProcessEvaluation15 class]])
            fitziProcessEvaluation15 = (FitziProcessEvaluation15 *)object;
    }
    [self addSubview:fitziProcessEvaluation15];
    [UIView animateWithDuration:1.0
                     animations:^{
                         ...
                         fitziProcessEvaluation15.frame = self.frame; // its final location
                     }];
}
...
- (IBAction) swipeDownDone:(id) sender
{
    ...
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"FitziProcessEvaluation13"
                                                    owner:self options:nil];
    FitziProcessEvaluation13 *indexView;
    for (id object in bundle) {
        if ([object isKindOfClass:[FitziProcessEvaluation13 class]])
            indexView = (FitziProcessEvaluation13 *)object;
    }
