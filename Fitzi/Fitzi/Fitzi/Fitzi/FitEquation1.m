...
- (IBAction) swipeDownDone:(id) sender
{
    ...
    equationView.frame = CGRectMake(0, 0, self.bounds.size.width,  self.bounds.size.height);
    [UIView animateWithDuration:1.0
                     animations:^{
                         equationView.frame = CGRectMake(0, 0, self.bounds.size.width,  self.bounds.size.height) ;//self.frame;
                     }];
}
