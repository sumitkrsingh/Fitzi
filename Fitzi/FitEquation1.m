...
-(void)awakeFromNib
{
    NSLog(@"fit equation mai aa gaya");
}
...
- (IBAction) swipeUpDone:(id) sender
{
    ...
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"FitEvolutionView_Additional"
                                                    owner:self options:nil];
    FitEvolutionView_Additional *indexView;
    for (id object in bundle) {
        if ([object isKindOfClass:[FitEvolutionView_Additional class]])
            indexView = (FitEvolutionView_Additional *)object;
    }
...
- (IBAction) swipeDownDone:(id) sender
{
    ...
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"FitEquation1_Additional"
                                                    owner:self options:nil];
    for (id object in bundle) {
        if ([object isKindOfClass:[FitEquation1_Additional class]])
            fitEquation1_Additional = (FitEquation1_Additional *) object;
    }
    [self addSubview:fitEquation1_Additional];
    fitEquation1_Additional.frame = CGRectMake(0.0, self.bounds.size.height, fitEquation1_Additional.bounds.size.width, fitEquation1_Additional.bounds.size.height);
    [UIView animateWithDuration:1.0
                     animations:^{
                         [UIView beginAnimations:@"page transition" context:nil];
                         [UIView setAnimationDuration:2.0];
                         ...
                         fitEquation1_Additional.frame = self.frame;//CGRectMake(12.0,24.0, equationView1.bounds.size.width, (equationView1.bounds.size.height)-20.0);
                     }];
}
