...
@implementation FitziProcessEvaluation7
...
- (IBAction) swipeUpDone:(id) sender
{
    ...
    for (id object in bundle) {
        if ([object isKindOfClass:[FitziProcessEvaluation8 class]])
            fitziProcessEvaluation8 = (FitziProcessEvaluation8 *)object;
    }
    [self addSubview:fitziProcessEvaluation8];
    fitziProcessEvaluation8.frame = CGRectMake(0.0, self.bounds.size.height, fitziProcessEvaluation8.bounds.size.width, fitziProcessEvaluation8.bounds.size.height);
    [UIView animateWithDuration:1.0
                     animations:^{
                         ...
                         fitziProcessEvaluation8.frame = self.frame; // its final location
                     }];
}
...
- (IBAction) swipeDownDone:(id) sender
{
    ...
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"FitziProcessEvaluation6"
                                                    owner:self options:nil];
    FitziProcessEvaluation6 *indexView;
    for (id object in bundle) {
        if ([object isKindOfClass:[FitziProcessEvaluation6 class]])
            indexView = (FitziProcessEvaluation6 *)object;
    }
