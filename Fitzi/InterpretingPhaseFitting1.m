...
@implementation InterpretingPhaseFitting1
...
- (IBAction) swipeUpDone:(id) sender
{
    ...
    for (id object in bundle) {
        if ([object isKindOfClass:[InterpretingPhaseFitting2 class]])
            evaluationView = (InterpretingPhaseFitting2 *)object;
    }
...
- (IBAction) swipeDownDone:(id) sender
{
    ...
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"InterpretingPhaseFitting"
                                                    owner:self options:nil];
    InterpretingPhaseFitting *indexView;
    for (id object in bundle) {
        if ([object isKindOfClass:[InterpretingPhaseFitting class]])
            indexView = (InterpretingPhaseFitting *)object;
    }
