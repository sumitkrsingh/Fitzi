...
#import "CapturePhase.h"

@implementation CapturePhase1
...
- (IBAction) swipeUpDone:(id) sender
{
    ...
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"CapturePhase2"
                                                    owner:self options:nil];
    for (id object in bundle) {
        if ([object isKindOfClass:[CapturePhase2 class]])
            evaluationView = (CapturePhase2 *)object;
    }
...
- (IBAction) swipeDownDone:(id) sender
{
    ...
    NSArray *bundle = [[NSBundle mainBundle] loadNibNamed:@"CapturePhase"
                                                    owner:self options:nil];
    CapturePhase *indexView;
    for (id object in bundle) {
        if ([object isKindOfClass:[CapturePhase class]])
            indexView = (CapturePhase *)object;
    }
